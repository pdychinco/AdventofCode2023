...
""";
    public static boolean isValidGame(String gameSets) {
        String[] gameDetails = gameSets.split(":");
        String gameNumber = gameDetails[0].split(" ")[0];
        String[] sets = gameDetails[1].split(";");
        for (String set: sets) {
            String[] colors = set.split(",");
            for (String color: colors) {
                color = color.trim();
                String[] count = color.split(" ");
                if(count[1].equalsIgnoreCase("red")) {
                    if(Integer.parseInt(count[0]) > red) {
                        return false;
                    }
                } else if (count[1].equalsIgnoreCase("blue")) {
                    if(Integer.parseInt(count[0]) > blue) {
                        return false;
                    }
                } else if (count[1].equalsIgnoreCase("green")) {
                    if(Integer.parseInt(count[0]) > green) {
                        return false;
                    }
                }
            }
    ...
    public static void main(String[] args) {
        ...
        for (String game : games
            ...
            if(isValidGame(game)) {
                ...
                total += Integer.parseInt(gameDetails[0].split(" ")[1]);
            }
        }
